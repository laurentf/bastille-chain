  ...
  describe "post-quantum key generation" do
    test "generates Dilithium keypairs" do
      try do
        ...
        # ML-DSA-44 verifying key is 1312 bytes; the private key is the 32-byte seed
        assert byte_size(public_key) == 1312
  ...
  describe "post-quantum signatures" do
    test "signs and verifies with Dilithium" do
      try do
        ...
        # Dilithium signatures are large
        assert byte_size(signature) > 1000
    ...
    test "signs and verifies with Falcon" do
      try do
        ...
        assert is_binary(signature)
        # Falcon signatures
        assert byte_size(signature) > 500
  ...
  describe "performance benchmarks" do
    ...
    test "post-quantum signature performance" do
      try do
        ...
        signatures =
          for _i <- 1..signing_iterations do
        ...
        signs_per_second = signing_iterations / (signing_duration / 1_000_000)
        ...
        verifications_per_second = verification_iterations / (verification_duration / 1_000_000)

        IO.puts(
          "Dilithium Verification: #{Float.round(verifications_per_second, 2)} verifications/second"
        )

        # Performance should be reasonable for blockchain use
        # At least 1 signature per second
        assert signs_per_second > 1
        # At least 10 verifications per second
    ...
    test "handles invalid keys for signatures" do
      try do
        ...
        try do
          ...
          ArgumentError -> :ok
          # Other errors are acceptable for invalid keys
          _ -> :ok
        end
