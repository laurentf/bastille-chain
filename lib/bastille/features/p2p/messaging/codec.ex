defmodule Bastille.Features.P2P.Messaging.Codec do
  ...
  defp decode_proto(frame_binary) do
    case Bastille.Features.P2P.Messaging.Envelope.decode(frame_binary) do
      %Bastille.Features.P2P.Messaging.Envelope{msg: {:version, v}} ->
        {:ok, {:version, from_version(v)}}

      %Bastille.Features.P2P.Messaging.Envelope{version: v} when not is_nil(v) ->
        {:ok, {:version, from_version(v)}}

      %Bastille.Features.P2P.Messaging.Envelope{msg: {:verack, _}} ->
        {:ok, {:verack, []}}

      %Bastille.Features.P2P.Messaging.Envelope{verack: %Proto.Verack{}} ->
        {:ok, {:verack, []}}

      %Bastille.Features.P2P.Messaging.Envelope{msg: {:ping, %Proto.Ping{nonce: n}}} ->
        {:ok, {:ping, n}}

      %Bastille.Features.P2P.Messaging.Envelope{ping: %Proto.Ping{nonce: n}} ->
        {:ok, {:ping, n}}

      %Bastille.Features.P2P.Messaging.Envelope{msg: {:pong, %Proto.Pong{nonce: n}}} ->
        {:ok, {:pong, n}}

      %Bastille.Features.P2P.Messaging.Envelope{pong: %Proto.Pong{nonce: n}} ->
        {:ok, {:pong, n}}
      ...
      %Bastille.Features.P2P.Messaging.Envelope{msg: {:getheaders, gh}} ->
        {:ok, {:getheaders, from_getheaders(gh)}}

      %Bastille.Features.P2P.Messaging.Envelope{getheaders: gh} when not is_nil(gh) ->
        {:ok, {:getheaders, from_getheaders(gh)}}

      %Bastille.Features.P2P.Messaging.Envelope{msg: {:headers, h}} ->
        {:ok, {:headers, from_headers(h)}}

      %Bastille.Features.P2P.Messaging.Envelope{headers: h} when not is_nil(h) ->
        {:ok, {:headers, from_headers(h)}}
      ...
      %Bastille.Features.P2P.Messaging.Envelope{msg: {:getblocks, gb}} ->
        {:ok, {:getblocks, from_getblocks(gb)}}

      %Bastille.Features.P2P.Messaging.Envelope{getblocks: gb} when not is_nil(gb) ->
        {:ok, {:getblocks, from_getblocks(gb)}}

      %Bastille.Features.P2P.Messaging.Envelope{
        msg: {:height, %Proto.Height{height: h, timestamp: t}}
      } ->
      ...
      %Bastille.Features.P2P.Messaging.Envelope{msg: {:block, b}} ->
        {:ok, {:block, from_block(b)}}

      %Bastille.Features.P2P.Messaging.Envelope{block: b} when not is_nil(b) ->
        {:ok, {:block, from_block(b)}}

      %Bastille.Features.P2P.Messaging.Envelope{msg: {:tx, t}} ->
        {:ok, {:tx, from_tx(t)}}

      %Bastille.Features.P2P.Messaging.Envelope{tx: t} when not is_nil(t) ->
        {:ok, {:tx, from_tx(t)}}
  ...
  defp to_envelope(:verack, _),
    do: {:ok, %Bastille.Features.P2P.Messaging.Envelope{verack: %Proto.Verack{}}}
  ...
  defp to_envelope(:ping, nonce),
    do: {:ok, %Bastille.Features.P2P.Messaging.Envelope{ping: %Proto.Ping{nonce: nonce}}}

  defp to_envelope(:pong, nonce),
    do: {:ok, %Bastille.Features.P2P.Messaging.Envelope{pong: %Proto.Pong{nonce: nonce}}}

  defp to_envelope(:getaddr, _),
    do: {:ok, %Bastille.Features.P2P.Messaging.Envelope{getaddr: %Proto.GetAddr{}}}

  defp to_envelope(:getheaders, payload) do
    {:ok,
     %Bastille.Features.P2P.Messaging.Envelope{
       getheaders: %Proto.GetHeaders{
         version: get_int(payload, ["version", :version], 1),
         hash_count: get_int(payload, ["hash_count", :hash_count], 1),
         block_locator_hashes:
           get_list(payload, ["block_locator_hashes", :block_locator_hashes], []),
  ...
  defp to_envelope(:headers, payload) do
    ...
    {:ok,
     %Bastille.Features.P2P.Messaging.Envelope{
       headers: %Proto.Headers{count: length(headers), headers: bin_headers}
     }}
  end
  ...
  defp to_inventory_item(%{type: t, hash: h}) do
    # No normalization - keep as-is
    %Proto.InventoryItem{type: to_item_type(t), hash: h}
  end
  ...
  defp to_block(%{"hash" => _} = m),
    do: to_block(%{hash: m["hash"], header: m["header"], transactions: m["transactions"]})

  defp to_block(%{hash: h, header: header, transactions: txs}) do
    %Proto.Block{
      # Keep hash as-is - no normalization
      hash: h,
  ...
  defp to_block_header(header) do
    ...
    %Proto.BlockHeader{
      ...
      # No normalization
      previous_hash: get_any(header, ["previous_hash", :previous_hash], <<0>>),
      timestamp: get_int(header, ["timestamp", :timestamp], 0),
      # No normalization
      merkle_root: get_any(header, ["merkle_root", :merkle_root], <<0>>),
  ...
  defp from_getheaders(%Proto.GetHeaders{} = gh) do
    %{
      "version" => gh.version,
      "hash_count" => gh.hash_count,
      "block_locator_hashes" => gh.block_locator_hashes,
      "hash_stop" => gh.hash_stop
    }
  end

  defp from_headers(%Proto.Headers{} = h) do
    decoded = Enum.map(h.headers, &safe_binary_to_term/1)
  ...
  defp from_getblocks(%Proto.GetBlocks{} = gb) do
    %{
      "version" => gb.version,
      "start_height" => gb.start_height,
      "stop_height" => gb.stop_height,
      "max_count" => gb.max_count
    }
  end
  ...
  # exhaustion (a remote DoS). `:safe` forbids new atoms (and refs/funs/pids).
